...
- The Nostr client supports three login methods: a browser extension signer (NIP-07) or remote signer (NIP-46), where your private key never touches this page at all, or pasting a private key (nsec) directly. If you use nsec, the key is encrypted at rest with a device-bound key (not plain text) by default, with an optional passphrase lock in Settings → Nostr for stronger protection.
