...
- If you log into the Nostr client with a private key (nsec), it's encrypted at rest with a device-bound key (not plain text), with an optional passphrase lock in Settings → Nostr for stronger protection at the cost of auto-login. Prefer a browser extension signer (NIP-07) or remote signer (NIP-46) when available — those never expose the key to this page at all.
